...
export const AuthModal: React.FC<AuthModalProps> = ({
  ...
  const handleLogin = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (result && result.success && loginUser) {
        if (result.token) {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    if (isUploadingPhoto) {
      setRegError('Mohon tunggu sampai proses foto selesai.');
      return;
    }
    ...
    try {
      ...
      const memberData = {
        ...
        avatarUrl: photoUrl,
        gender: regGender,
        birthPlace: regBirthPlace.trim() || regName,
        birthDate: regBirthDate,
        phone,
        email,
        address: `${distName}, ${regName}`,
        provinceId: isNasional ? '00' : regProvinceId,
        provinceName: provName,
        regencyId: isNasional ? '00.00' : regRegencyId,
        regencyName: regName,
        districtId: isNasional ? '00.00.00' : regDistrictId,
        ...
        bio: regBio.trim() || 'Calon anggota Saka Pariwisata yang siap memajukan pariwisata nusantara.',
        skills: [],
        certifications: []
      };
  ...
  const handleResetPassword = async (e: React.FormEvent) => {
    ...
    if (!forgotOtp.trim()) {
      setForgotError('Masukkan kode pemulihan yang dikirim ke email.');
      return;
    }
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-2.5">
                  <div>
                    ...
                    <select
                      value={regGender}
                      onChange={e => setRegGender(e.target.value as 'LAKI_LAKI' | 'PEREMPUAN')}
                      className="w-full px-3 py-2 bg-slate-50 border border-slate-300 rounded-xl focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 outline-none text-slate-800"
                    >
              ...
              <div className="space-y-3 pt-1">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-2.5">
                  <div>
                    ...
                    <select
                      value={regKrida}
                      onChange={e => setRegKrida(e.target.value as KridaType)}
