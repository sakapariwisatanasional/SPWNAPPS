    ...
    <div className="bg-white w-full max-w-7xl max-h-[96vh] rounded-3xl shadow-2xl overflow-hidden flex flex-col">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-12 flex-1 min-h-0">
        <div className="xl:col-span-8 p-5 overflow-y-auto space-y-5">
          ...
          {side==='FRONT' && <section className="p-4 rounded-2xl border border-blue-200 bg-blue-50/40 space-y-3">
            ...
            <div className="grid grid-cols-2 md:grid-cols-5 gap-2"><label className="text-[9px] font-bold">X{numberInput((settings as any).barcodeFrontX??4,v=>setSettings(s=>({...s,barcodeFrontX:v})))} </label><label className="text-[9px] font-bold">Y{numberInput((settings as any).barcodeFrontY??77,v=>setSettings(s=>({...s,barcodeFrontY:v})))} </label><label className="text-[9px] font-bold">Lebar{numberInput((settings as any).barcodeFrontWidth??32,v=>setSettings(s=>({...s,barcodeFrontWidth:v})))} </label><label className="text-[9px] font-bold">Tinggi{numberInput((settings as any).barcodeFrontHeight??9,v=>setSettings(s=>({...s,barcodeFrontHeight:v})))} </label><label className="text-xs font-bold flex items-center gap-2"><input type="checkbox" checked={(settings as any).barcodeFrontShowText===true} onChange={e=>setSettings(s=>({...s,barcodeFrontShowText:e.target.checked}))}/> Tampilkan angka</label></div>
