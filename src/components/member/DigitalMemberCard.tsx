      ...
      <div className="relative w-full h-full transition-transform duration-500" style={{transformStyle:'preserve-3d',transform:flipped?'rotateY(180deg)':'none'}}>
        ...
        <div className="absolute inset-0 overflow-hidden shadow-2xl border border-white/20 text-white p-4" style={{...bgStyle(bgBack,settings.customBackgroundColorBack),borderRadius:radius,backfaceVisibility:'hidden',transform:'rotateY(180deg)'}}>
          ...
          {(settings as any).showSignerQrCode !== false && (() => {
            const signerId = (settings as any).signerMemberId;
          ...
          {(settings as any).showSignerVerified !== false && (settings as any).signerMemberId && (
            <div className="absolute flex items-center gap-1" style={{left:`${(settings as any).signerVerifiedX ?? 5}%`,top:`${(settings as any).signerVerifiedY ?? 62}%`,width:`${(settings as any).signerVerifiedWidth ?? 55}%`,color:(settings as any).signerVerifiedColor ?? '#ffffff',fontSize:`${(settings as any).signerVerifiedFontSize ?? 7}px`,fontWeight:700,lineHeight:1.15}}>
              <ShieldCheck className="w-2.5 h-2.5 flex-none"/>PENANDATANGAN TERVERIFIKASI
            </div>
          )}
        </div>
      </div>
