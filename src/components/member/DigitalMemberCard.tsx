    ...
    <div style={{width:widthPx,height:heightPx,perspective:'1000px'}} className="cursor-pointer" onClick={()=>setFlipped(v=>!v)}>
      <div className="relative w-full h-full transition-transform duration-500" style={{transformStyle:'preserve-3d',transform:flipped?'rotateY(180deg)':'none'}}>
        <div className="absolute inset-0 overflow-hidden shadow-2xl border border-white/20 text-white" style={{...bgStyle(bgFront,settings.customBackgroundColorFront),borderRadius:radius,backfaceVisibility:'hidden'}}>
          ...
          {(settings as any).showBarcodeFront !== false && <div className="absolute flex flex-col items-center gap-1" style={{left:`${(settings as any).barcodeFrontX ?? 4}%`,top:`${(settings as any).barcodeFrontY ?? 77}%`,width:`${(settings as any).barcodeFrontWidth ?? 32}%`}}><div className="bg-white rounded p-1 w-full"><Barcode value={(settings as any).barcodeFrontCustomValue?.trim()||member.nationalMemberNumber||member.id} width={Math.max(40,Math.round(widthPx*.25))} height={Math.max(12,Math.round(heightPx*((settings as any).barcodeFrontHeight ?? 9)/100))} barColor="#000" showText={(settings as any).barcodeFrontShowText === true}/></div>{(settings as any).barcodeFrontCaption && <div className="text-[6px] flex items-center gap-1"><ShieldCheck className="w-2.5 h-2.5"/>{(settings as any).barcodeFrontCaption}</div>}</div>}
