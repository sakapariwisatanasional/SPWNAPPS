...
const DigitalMemberCard: React.FC<DigitalMemberCardProps> = ({
  member,
  currentUser,
}) => {
  ...
  return (
    <section className="relative overflow-hidden rounded-[2rem] p-6 md:p-8 text-white bg-gradient-to-br from-red-600 via-amber-400 to-teal-500 shadow-xl">
      ...
      <div className="relative">
          ...
          <div className="flex items-center gap-4">
            {photo ? (
              <img
                src={photo}
                alt={name}
                className="w-20 h-20 rounded-2xl object-cover border-2 border-white/50"
              />
            ) : (
        ...
        <div className="mt-6 text-xs font-bold opacity-80">
          DIGITAL MEMBER CARD
        </div>
      </div>
    </section>
