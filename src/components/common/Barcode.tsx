...
function encodeCode128B(value: string): string {
  ...
  const values = Array.from(input, (ch) => ch.charCodeAt(0) - 32);
  const checksum =
    (104 + values.reduce((sum, v, index) => sum + v * (index + 1), 0)) % 103;
...
export const Barcode: React.FC<BarcodeProps> = ({
  ...
  return (
    <div className={`inline-flex flex-col items-center select-none ${className}`}>
      ...
      >
        {bgColor !== 'transparent' && (
          <rect x="0" y="0" width={encoded.length} height={height} fill={bgColor} />
        )}
        {encoded.split('').map((bit, index) =>
          bit === '1'
            ? <rect key={index} x={index} y="0" width="1" height={height} fill={barColor} />
            : null
        )}
      </svg>
      {showText && (
