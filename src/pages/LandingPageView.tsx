      ...
      <div className="spwn-wi-glow spwn-wi-glow-a" /><div className="spwn-wi-glow spwn-wi-glow-b" /><div className="spwn-wi-glow spwn-wi-glow-c" /><header className="sticky top-0 z-50 border-b border-white/5 bg-[#8f72cf]/65 backdrop-blur-xl">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between gap-3">
          <button type="button" onClick={() => scrollTo('landing-top')} className="flex items-center gap-2.5 min-w-0 cursor-pointer">
            ...
            <div className="text-left min-w-0">
              <div className="font-black text-sm sm:text-base tracking-wide truncate">SAKA <span className="text-white">PARIWISATA</span></div>
              <div className="hidden sm:block text-[9px] uppercase tracking-[.16em] text-white/70">Kwartir Nasional Gerakan Pramuka</div>
            </div>
          </button>
        ...
        {mobileMenuOpen && (
          <div className="sm:hidden border-t border-white/5 bg-[#8f72cf] px-4 py-3 grid grid-cols-3 gap-2">
            ...
            {currentUser?.role === 'PUBLIC' ? <button type="button" onClick={onOpenLoginModal} className="rounded-2xl border border-white/5 bg-white/[.03] p-3 text-center cursor-pointer"><LockKeyhole className="w-5 h-5 mx-auto mb-1 text-white/90" /><span className="text-[10px] font-bold">Masuk</span></button> : <button type="button" onClick={() => onEnterDashboard(currentUser.role === 'MEMBER' ? 'my-card' : 'dashboard')} className="rounded-2xl border border-white/5 bg-white/[.03] p-3 text-center cursor-pointer"><LayoutDashboard className="w-5 h-5 mx-auto mb-1 text-white/90" /><span className="text-[10px] font-bold">Panel</span></button>}
          </div>
        )}
      </header>
      ...
      <main id="landing-top">
            ...
            <div className="mt-8 grid grid-cols-3 sm:grid-cols-6 gap-2.5 sm:gap-3 max-w-4xl mx-auto">
              {HOME_TOOLS.map(tool => {
                ...
                return <button key={tool.id} type="button" onClick={() => openTool(tool.id)} className={`spwn-wi-tool group rounded-[22px] p-3 sm:p-4 transition-all cursor-pointer ${active ? 'spwn-wi-tool-active' : ''}`}>
                  ...
                  <div className="hidden sm:block text-[9px] text-white/70 mt-0.5">{tool.hint}</div>
                </button>;
              })}
            </div>
          </div>
        ...
        <section className="px-4 sm:px-6 pb-12">
          <div className="max-w-6xl mx-auto grid grid-cols-3 gap-2.5 sm:gap-4">
            <div className="spwn-wi-card rounded-[22px] p-3 sm:p-5 text-center"><Users className="w-4 h-4 mx-auto text-cyan-300 mb-2" /><div className="text-xl sm:text-3xl font-black">{activeMembersCount}</div><div className="text-[9px] sm:text-xs text-white/70 mt-1">Anggota aktif</div></div>
            <div className="spwn-wi-card rounded-[22px] p-3 sm:p-5 text-center"><Compass className="w-4 h-4 mx-auto text-amber-300 mb-2" /><div className="text-xl sm:text-3xl font-black">{publishedTours.length}</div><div className="text-[9px] sm:text-xs text-white/70 mt-1">Paket wisata</div></div>
            <div className="spwn-wi-card rounded-[22px] p-3 sm:p-5 text-center"><Gift className="w-4 h-4 mx-auto text-rose-300 mb-2" /><div className="text-xl sm:text-3xl font-black">{approvedProducts.length}</div><div className="text-[9px] sm:text-xs text-white/70 mt-1">Karya produk</div></div>
          </div>
        </section>
          ...
          <div className="max-w-6xl mx-auto">
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
              {KRIDA_CATEGORIES.map(category => {
                ...
                return <button key={category.id} type="button" onClick={() => openKrida(category.id as KridaId)} className="spwn-wi-card text-left rounded-[28px] hover:border-fuchsia-400/25 p-4 sm:p-5 transition-all cursor-pointer group"><div className="flex items-start justify-between"><div className="spwn-wi-icon w-11 h-11"><Icon className="w-5 h-5 text-purple-300" /></div><ChevronRight className="w-4 h-4 text-white/60 group-hover:text-purple-300 transition-transform group-hover:translate-x-0.5" /></div><h3 className="mt-4 text-sm sm:text-base font-black">{category.name}</h3><p className="mt-1 text-[10px] sm:text-xs text-white/70 line-clamp-2">{category.description}</p><div className="mt-4 text-[9px] font-bold text-purple-300">{count || category.topicsCount} materi</div></button>;
              })}
            </div>
          </div>
        </section>
        ...
        <section id="landing-discover" className="scroll-mt-20 px-4 sm:px-6 py-12">
          <div className="max-w-6xl mx-auto">
            ...
            {publishedTours.length > 0 ? <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 mb-6">{publishedTours.slice(0, 3).map(tour => <button key={tour.id} type="button" onClick={() => onViewTourDetail(tour)} className="spwn-wi-card text-left rounded-[28px] overflow-hidden hover:border-amber-400/25 transition-all cursor-pointer group"><div className="h-40 bg-slate-900 overflow-hidden"><img src={formatDriveImageUrl(tour.imageUrl) || 'https://images.unsplash.com/photo-1518709268805-4e9042af9f23?w=1000&auto=format&fit=crop&q=80'} alt={tour.title} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" loading="lazy" referrerPolicy="no-referrer" /></div><div className="p-4"><div className="flex items-center gap-1 text-[9px] text-white/70"><MapPin className="w-3 h-3" /> {tour.regencyName}, {tour.provinceName}</div><h3 className="mt-2 text-sm font-black line-clamp-2">{tour.title}</h3></div></button>)}</div> : <div className="rounded-2xl border border-white/5 bg-white/[.02] p-5 text-xs text-white/70 mb-6">Belum ada paket wisata yang dipublikasikan.</div>}
