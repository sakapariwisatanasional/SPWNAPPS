...
export const PublicVerificationPage: React.FC<PublicVerificationPageProps> = ({ members }) => {
  ...
  const verifiedTermRef = useRef<string | null>(null);
  ...
  const verifyFromUrl = async () => {
    ...
    if (!term) return;
    const attemptKey = `${term}:${members.length}`;
    if (verifiedTermRef.current === attemptKey) return;
    verifiedTermRef.current = attemptKey;
    ...
    try {
      const verification = await verifyMemberUniversal(term, members, { authoritativeRemote: true });
      ...
      setError(err?.message || 'Terjadi kesalahan saat memverifikasi data anggota.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const avatar = member
    ? formatDriveImageUrl(member.avatarUrl) || member.avatarUrl || getValidAvatarUrl(member.avatarUrl, member.gender)
    : '';
      ...
      <header className="border-b border-slate-200 bg-white/95 backdrop-blur-sm sticky top-0 z-20">
        <div className="max-w-3xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between gap-4">
          <div className="flex items-center gap-3 min-w-0">
            <SakaLogo size={40} id="public-verification-logo" />
            <div className="min-w-0">
              <p className="text-[10px] font-bold uppercase tracking-widest text-purple-700">Verifikasi KTA</p>
              <p className="text-sm font-extrabold text-slate-900 truncate">Saka Pariwisata Kwartir Nasional</p>
            </div>
          </div>
          <button
            type="button"
        ...
        <div className="w-full max-w-2xl">
          {loading ? (
            <div className="bg-white rounded-[2rem] border border-slate-200 shadow-sm p-8 sm:p-12 text-center">
              ...
              <h1 className="mt-5 text-xl font-extrabold font-heading">Memverifikasi KTA...</h1>
              <p className="mt-2 text-sm text-slate-500">Mohon tunggu, data sedang dicocokkan dengan sumber resmi.</p>
            </div>
          ) : member ? (
            <div className="bg-white rounded-[2rem] border border-slate-200 shadow-xl overflow-hidden">
              <div className={`px-6 py-7 sm:px-8 text-white text-center relative overflow-hidden ${
                isVerified
                ...
                <div className="relative">
                  <div className="mx-auto w-14 h-14 rounded-2xl bg-white/10 border border-white/15 flex items-center justify-center mb-4">
                    {isVerified ? <ShieldCheck className="w-8 h-8 text-emerald-300" /> : <XCircle className="w-8 h-8 text-amber-300" />}
                  </div>
                  <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-white/10 border border-white/15 text-[11px] font-extrabold uppercase tracking-wider">
                    {isVerified ? <CheckCircle2 className="w-3.5 h-3.5 text-emerald-300" /> : <XCircle className="w-3.5 h-3.5 text-amber-300" />}
                    {isVerified ? 'Anggota Resmi Terverifikasi' : 'Status Keanggotaan'}
                  </span>
                  <h1 className="mt-3 text-2xl sm:text-3xl font-extrabold font-heading">Hasil Verifikasi KTA</h1>
                  <p className="mt-1 text-xs text-purple-200/80">Sistem Verifikasi Digital Nasional Kwartir Gerakan Pramuka</p>
                </div>
              </div>
              ...
              <div className="p-5 sm:p-8 space-y-6">
                <div className="flex flex-col sm:flex-row items-center sm:items-start gap-5 p-5 rounded-[1.5rem] bg-purple-50/60 border border-purple-100">
                  ...
                  <div className="text-center sm:text-left min-w-0 flex-1">
                    <p className="text-[10px] font-bold uppercase tracking-widest text-purple-700">Nama Anggota</p>
                    <h2 className="mt-1 text-2xl font-extrabold font-heading text-slate-900 break-words">{member.fullName}</h2>
                    <div className="mt-2 inline-flex items-center gap-2 px-3 py-1.5 rounded-xl bg-white border border-purple-100 text-purple-950 font-mono text-sm font-extrabold">
                      ...
                      {member.nationalMemberNumber || 'Nomor Anggota Dalam Proses'}
                    </div>
                    <p className="mt-2 text-xs text-slate-500 font-medium">
                      {member.currentPosition || 'Anggota Saka'}{member.krida ? ` • ${member.krida}` : ''}
                    </p>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                  <div className="p-4 bg-slate-50 rounded-2xl border border-slate-100">
                    ...
                    <p className="mt-1 text-sm font-bold text-slate-800">{member.provinceName || '-'}</p>
                  </div>
                  <div className="p-4 bg-slate-50 rounded-2xl border border-slate-100">
                    <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Kabupaten/Kota</p>
                    <p className="mt-1 text-sm font-bold text-slate-800">{member.regencyName || '-'}</p>
                  </div>
                  <div className="p-4 bg-slate-50 rounded-2xl border border-slate-100">
                    <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Kwartir Ranting</p>
                    <p className="mt-1 text-sm font-bold text-slate-800">{member.districtName || '-'}</p>
                  </div>
                </div>
                  ...
                  <section>
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {member.skills.map((skill) => (
                        <span key={skill.id} className="px-2.5 py-1 bg-purple-50 text-purple-900 border border-fuchsia-200/80 rounded-lg text-xs font-semibold">
                          {skill.skillName} • <span className="text-fuchsia-600 font-bold">{skill.proficiency}</span>
                        </span>
                      ))}
                    </div>
                  </section>
                ...
                <div className="flex items-center gap-3 p-4 rounded-2xl bg-emerald-50 border border-emerald-100">
                  ...
                  <div>
                    <p className="text-xs font-extrabold text-emerald-900">Data ditemukan pada sistem verifikasi</p>
                    <p className="text-[11px] text-emerald-700 mt-0.5">Informasi yang ditampilkan adalah data publik yang diperuntukkan untuk keperluan verifikasi KTA.</p>
                  </div>
                </div>

                {showCard && (
                  <div className="pt-2 border-t border-slate-200">
                    <p className="text-xs text-slate-500 text-center mb-4">KTA Digital Anggota</p>
                ...
                <div className="flex flex-col sm:flex-row gap-3 pt-1">
                  ...
                  >
                    ...
                    {showCard ? 'Sembunyikan KTA Digital' : 'Lihat KTA Digital'}
                  </button>
                  <button
                    type="button"
                    onClick={goBack}
                ...
                <p className="text-[10px] text-center text-slate-400 leading-relaxed">
                  Informasi privat seperti NIK, alamat lengkap, dan kontak pribadi tidak ditampilkan pada halaman verifikasi publik.
                </p>
              </div>
            </div>
          ) : (
            <div className="bg-white rounded-[2rem] border border-rose-200 shadow-sm p-8 sm:p-12 text-center">
              ...
              <h1 className="mt-5 text-xl font-extrabold font-heading">Data Anggota Tidak Ditemukan</h1>
